...
object OrderEndpointSpec extends ZIOSpecDefault:
  ...
  def spec = suite("Order Endpoint with Embedded Kafka")(
    ...
    test(
      "GET /check/:orderId should return 303 See Other when order is ready"
    ) {
    ...
    test(
      "GET /pickup/:orderId should return the order when it exists and remove it"
    ) {
    ...
    test(
      "Full workflow: order -> check (not ready) -> ready -> check (redirect) -> pickup"
    ) {
      for
    ...
    KafkaTestUtils.consumer(
      clientId = "coffeebar-test",
